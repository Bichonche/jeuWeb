...
var player = {
    init : function(){
		...
		for (j = 0 ; j < 4 ; j++ ) {
            ...
            this.tmpCanvas.setAttribute("width",this.width);
            this.tmpCanvas.setAttribute("height", this.height);
            this.tmpCtxc = this.tmpCanvas.getContext("2d");
			this.img.ready = false;
			this.img.onload = function (){
				this.tmpCanvas.drawImage(this.img, 0, j * this.height, this.width, this.height, 0, 0, this.width, this.height);
				player.ready = true;
			}
		this.offScreenCanvas.push(this.tmpCanvas);
        }
    },
    x : 20,
    ySpeed : 5,
	...
	for (keycode in keyStatus) {
		if(keyStatus[keycode] == true){
			...
			if(keycode == keys.DOWN) {
				player.y += player.ySpeed;
				if(player.y>ArenaHeight-player.height) player.y=ArenaHeight-player.height;
			}	
...
function gameOver() {
	...
	canGO = document.createElement("canvas");
	canGO.id = "gameOver";
}
