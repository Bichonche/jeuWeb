            ...
            for (keycode in keyStatus) {
                if(keyStatus[keycode] == true){
                    ...
                    if(keycode == keys.SPACE) {
                        //shoot
                        //window.setTimeout(this.fires,200);
                    	if (delayProjectiles % 20 === 0) {this.fires();}
                    	delayProjectiles++;
                    	console.log(delayProjectiles);
                    }
                    delayProjectiles = 0;
                }
             //keyStatus[keycode] = false;
...
function drawScore() {
	conScore.fillText("Lives : "+player.nbOfLives, 10, 25);
	conScore.fillText("Score : "+player.projectileSet.score, 150,25);
}

function gameOver() {
	clearGame();
	conScore.fillText("Game Over", 300,500);
}
function updateGame() {
    "use strict"; 
    updateScene();
